...
class BaseService {
    ...
    @discardableResult
    func callEndPoint(endPoint: String, method: Alamofire.HTTPMethod = .get, headers: [String:String]? = [:], params: JsonDictionay? = [:], completion: @escaping (ServiceResponse) -> Void)->DataRequest?{
        
        let url = AppConstants.ServerURL.hedlineURL
